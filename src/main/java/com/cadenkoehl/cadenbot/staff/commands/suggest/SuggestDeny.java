...
public class SuggestDeny extends ListenerAdapter {
    @Override
    public void onGuildMessageReceived(@NotNull GuildMessageReceivedEvent event) {
        String[] args = event.getMessage().getContentRaw().split("\\s+");
        String guildId = event.getGuild().getId();
        String prefix = Constants.getPrefix(guildId);
        if(args[0].equalsIgnoreCase(prefix + "deny")) {
            ...
            event.getChannel().sendMessage(":white_check_mark: **Success**! Suggestion #" + number + " from **" + sug.getMember().getEffectiveName() + "** was denied!").queue();
        }
    }
}
